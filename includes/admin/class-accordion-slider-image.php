...
class Accordion_Slider_Image{
	...
	public function get_image_size( $id, $img_size, $type = 'creative-gallery', $sizes = array() ) {
		...
		if ( $image_full ) {
			...
			return new WP_Error( 'accordion-slider-gallery-error-no-url', esc_html__( 'No image with this ID.', 'accordion-slider' ) );
		}
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'accordion-slider-gallery-error-no-url', esc_html__( 'No image URL specified for cropping.', 'accordion-slider' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'accordion-slider-error-no-file', esc_html__( 'No file could be found for the image URL specified.', 'accordion-slider' ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'accordion-slider-error-no-size', esc_html__( 'The dimensions of the original image could not be retrieved for cropping.', 'accordion-slider' ) );
        }
        ...
        if ( $width && ! $height ) {
            ...
            return new WP_Error( 'accordion-slider-error-no-file', esc_html__( 'The dimensions of the original image could not be retrieved for cropping.', 'accordion-slider' ) );
        }
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = false, $align = 'c', $quality = 100, $retina = false, $data = array(), $force_overwrite = false ) {
        ...
        if ( ! file_exists( $dest_file_name ) || ( file_exists( $dest_file_name ) && $force_overwrite ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'accordion-slider-error-no-editor', esc_html__( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', 'accordion-slider' ) );
            }

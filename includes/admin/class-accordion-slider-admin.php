...
class Accordion_Slider_Admin {
	...
	function __construct() {
		...
		// Show general tab
		add_action( 'accordion_slider_admin_tab_general', array( $this, 'show_general_tab' ) );

		// Add CSS to admin menu
		add_action( 'admin_head', array( $this, 'admin_custom_css' ) );

		add_action( 'wp_ajax_accordion_slider_save_images', array( $this, 'save_images' ) );
	...
	public function save_images(){
		...
		if ( ! wp_verify_nonce( $nonce, 'accordion-slider-ajax-save' ) ) {
		...
		if ( 'accordion_slider' != get_post_type( $gallery_id ) ) {
			wp_send_json( array( 'status' => 'failed' ) );
		}

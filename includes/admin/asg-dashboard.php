...
$plugins_data	= asg_get_plugin_data();
$plugins_filter = asg_plugins_filter();

// Check Plugin Install Permission
if( ! current_user_can('install_plugins') ) {
	echo '<div class="error">
			<p>'. esc_html__( "Sorry, It looks like that you do not have permission to install the plugin.", 'accordion-slider') .'</p>
			<p>'. esc_html__("You can take a look at our all plugins at", 'accordion-slider') .' <a href="https://profiles.wordpress.org/wpdiscover#content-plugins" target="_blank">'. esc_html__("here", 'accordion-slider') . '</a>.</p>
		 </div>';
	return;
}
?>
...
<div class="wrap asg-settings">
	<div class="asg-dashboard-wrap">

		<?php if( ! empty( $plugins_data->plugins ) ) { ?>
		<form id="plugin-filter" method="post">
			<div class="asg-plugin-list-wrap">
				<div class="widefat asg-plugin-list asg-clearfix row" id="the-list">
					...
					<div class="asg-plugin-card-wrap <?php echo $extra_class; ?> col-md-6" data-tags="<?php echo esc_attr( $plugin_tags ); ?>">
						<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin_data['slug'] ); ?>">
							<div class="plugin-card-top">
								<div class="name column-name">
									<h3>
										<a href="<?php echo esc_url( $details_link ); ?>" class="thickbox open-plugin-details-modal">
											<span class="asg-plugin-name"><?php echo $title; ?></span>
					...
					</div><!-- end .asg-plugin-card-wrap -->

					<?php } ?>

				</div>
				
			</div><!-- end .asg-plugin-list-wrap -->			
		</form>
		<?php } else { ?>

				<div class="asg-no-result">
